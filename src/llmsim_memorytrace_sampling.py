...
def is_detail_question(q: str) -> bool:
    # crude but helpful: pushes IDK when the question looks detail-y
    ql = q.lower()
...
# OpenAI calls (Responses API + Structured Outputs JSON schema)
...
# Dual-trace memory: gist vs schema-based recollection
...
# Model outputs a small distribution; *we* sample final in code (VS-style).
ANSWER_DIST_SCHEMA = {
    "type": "object",
    "properties": {
        "candidates": {
...
Classify whether a typical non-expert student truly knows the answer from immediate memory.

Definitions:
- obvious: extremely common (basic school facts, very famous trivia) and instantly recalled with no thinking.
- maybe_heard: sounds somewhat familiar but not reliably recalled.
- technical_or_unknown: needs specialized knowledge, specific study facts, or careful reasoning.
...
- If you are not in the 'obvious' bucket, you should usually choose IDK, but occasionally make a quick familiarity guess.

Verbalized Sampling:
- Provide exactly 3 candidate option numbers and probabilities (not all mass on one).
- Include IDK as a candidate if you are unsure.
...
- IDK is a candidate if interpretation confidence is low OR nothing matches.
        ...
        for doc_id, doc in enumerate(raw):
            ...
            for q_idx, q_ref in enumerate(qs):
                ...
                for ann in anns:
                    ...
                    # ---------------- PRE (2-step: gate then dist then sample) ----------------
                    # Gate sees question only (no options)
                    ...
                    # Now ask for a distribution over options (includes IDK when unsure)
                    ...
                    if ha_post is not None and media in ("news", "abstract", "tweet"):
                        try:
                            if media == "news":
                                ...
                                # sample which trace is recalled
