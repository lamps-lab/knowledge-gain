...
# --- MODIFIED: Generators now receive the Questions to maximize KG ---
def build_simple_prompt(abstract: str, qas: List[Dict[str, Any]], news_length: int) -> Tuple[str, str]:
    system = (
    ...
    user = (
        f"ABSTRACT:\n{abstract}\n\n"
        f"CRITICAL REQUIREMENT:\nThe following questions will be used to test a reader's knowledge gain after reading your article. "
        f"Your article MUST contain all the information necessary to answer them correctly.\n\n"
        f"QUESTIONS TO COVER:\n{qas_to_text(qas)}\n\n"
        f"Write a science news article of about {news_length} words based on the abstract. "
        f"Ensure it is engaging but strictly grounded in the abstract."
    )
    return system, user

def build_drafter_prompt(abstract: str, qas: List[Dict[str, Any]], news_length: int) -> Tuple[str, str]:
    system = (
        "You are an expert science journalist. "
        "Your job is to take complex academic abstracts and turn them into engaging news articles. "
        "Output only the article text."
    )
    user = (
        f"ABSTRACT:\n{abstract}\n\n"
        f"KNOWLEDGE GOAL:\nYour article must empower a reader to answer these specific questions:\n{qas_to_text(qas)}\n\n"
        f"Please draft a news article in about {news_length} words based on the abstract. "
        f"Make the science easy to understand without introducing facts not found in the abstract."
    )
    return system, user
...
def build_revision_prompt(abstract: str, draft: str) -> Tuple[str, str]:
    system = (
        ...
        "on the original abstract. Output only the final polished article text."
    )
    user = f"ORIGINAL ABSTRACT:\n{abstract}\n\nINITIAL DRAFT:\n{draft}\n\nPlease revise and polish the draft. Ensure no scientific inaccuracies were introduced, improve the hook, and output ONLY the final polished article including the headline."
    return system, user
...
# --- MODIFIED: Get already processed abstracts to resume safely ---
def get_processed_abstracts(out_path: str) -> set:
    processed = set()
    if os.path.exists(out_path):
        with open(out_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    row = json.loads(line)
                    processed.add(row.get("paper_abstract", "").strip())
                except:
                    pass
    return processed
...
def main():
    ...
    # --- MODIFIED: Robust JSON Loading (ignores empty lines) ---
    records = []
    ...
    processed_abstracts = get_processed_abstracts(args.out)
    if processed_abstracts:
        print(f"Resuming: Found {len(processed_abstracts)} completed abstracts in output.")
    ...
    gen_model = AutoModelForCausalLM.from_pretrained(
        args.gen_model, torch_dtype=dtype, trust_remote_code=True,
    ...
    # Open in Append mode ("a") to enable resuming
    with open(args.out, "a", encoding="utf-8") as w:
        for idx, ex in enumerate(records):
            ...
            for sidx in range(args.simple_samples):
                # Now passing QAs to the Simple Generator
                sp, up = build_simple_prompt(abstract, qas, args.news_length)
                ...
                candidates.append({
                    "source": "simple", "sample_idx": sidx, "article": art, "scores": scores, "judge_raw": judge_raw, "weighted_total": total,
                })

            for aidx in range(args.agentic_samples):
                # Now passing QAs to the Drafter Generator
                sp, up = build_drafter_prompt(abstract, qas, args.news_length)
                ...
                total = weighted_total(scores, args.acc_weight, args.comp_weight, args.rel_weight, args.cla_weight, args.kg_weight) if scores else -9.0
                candidates.append({
                    "source": "agentic", "sample_idx": aidx, "article": art, "scores": scores, "judge_raw": judge_raw, "weighted_total": total,
                })
            ...
            w.flush() # Ensure it writes to disk immediately
            print(f"[{idx+1}/{len(records)}] ID={article_id} best={best['source']} total={best['weighted_total']:.3f}", flush=True)
